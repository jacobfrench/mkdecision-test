...
moqui.notifyMessages=function(a,b,c){var d=!1;if(a)if(moqui.isArray(a))for(var e=0;e<a.length;e++){var f=a[e];moqui.isPlainObject(f)?((d=moqui.getQuasarColor(f.type))&&d.length||(d="info"),moqui.webrootVue.$q.notify($.extend({},moqui.notifyOptsInfo,{type:d,message:f.message})),moqui.webrootVue.addNotify(f.message,d)):(moqui.webrootVue.$q.notify($.extend({},moqui.notifyOptsInfo,{message:f})),moqui.webrootVue.addNotify(f,"info"));d=!0}else moqui.webrootVue.$q.notify($.extend({},moqui.notifyOptsInfo,
{message:f})),moqui.webrootVue.addNotify(a,"info"),d=!0;if(b)if(moqui.isArray(b))for(a=0;a<b.length;a++)moqui.webrootVue.$q.notify($.extend({},moqui.notifyOptsError,{message:b[a]})),moqui.webrootVue.addNotify(b[a],"negative"),d=!0;else moqui.webrootVue.$q.notify($.extend({},moqui.notifyOptsError,{message:b})),moqui.webrootVue.addNotify(b,"negative"),d=!0;if(c)if(moqui.isArray(c))for(b=0;b<c.length;b++)moqui.notifyValidationError(c[b]),d=!0;else moqui.notifyValidationError(c),d=!0;return d};
...
moqui.handleAjaxError=function(a,b,c,d){if(!d){if("blob"===a.responseType){var e=new FileReader;e.onload=function(d){moqui.handleAjaxError(a,b,c,d.target.result)};e.readAsText(a.response);return}d=a.responseText}try{e=JSON.parse(d)}catch(g){}console.warn("ajax "+b+" ("+a.status+"), message "+c);var f=!1;401===a.status?f=moqui.notifyMessages(null,"No user authenticated"):e&&moqui.isPlainObject(e)?(f=moqui.notifyMessages(e.messageInfos,e.errors,e.validationErrors),console.log("got here notified ",f)):
d&&moqui.isString(d)&&d.length&&(f=moqui.notifyMessages(d));401===a.status?moqui.webrootVue?window.location.href=moqui.webrootVue.currentLinkUrl:window.location.reload(!0):0===a.status?0>c.indexOf("abort")&&(moqui.webrootVue.$q.notify($.extend({},moqui.notifyOptsError,{message:"Could not connect to server"})),moqui.webrootVue.addNotify("Could not connect to server","negative")):f||(console.log("got here 2 notified ",f),e="Error: "+c+" ("+b+")",moqui.webrootVue.$q.notify($.extend({},moqui.notifyOptsError,
{message:e})),moqui.webrootVue.addNotify(e,"negative"))};moqui.notifyGrowl=function(a){a&&(moqui.webrootVue.$q.notify($.extend({},moqui.notifyOptsInfo,{type:a.type,message:a.title})),moqui.webrootVue.addNotify(a.title,a.type))};moqui.componentCache=new moqui.LruMap(50);moqui.handleLoadError=function(a,b,c){"abort"===b?console.warn("load aborted: "+b+" ("+a.status+"), message "+c):(moqui.webrootVue.loading=0,moqui.handleAjaxError(a,b,c))};
...
Vue.component("m-form",{name:"mForm",props:{fieldsInitial:Object,action:{type:String,required:!0},method:{type:String,"default":"POST"},submitMessage:String,submitReloadId:String,submitHideId:String,focusField:String,noValidate:Boolean},data:function(){return{fields:Object.assign({},this.fieldsInitial),fieldsChanged:{},buttonClicked:null}},template:'\x3cq-form ref\x3d"qForm" @submit.prevent\x3d"submitForm" @reset.prevent\x3d"resetForm" autocapitalize\x3d"off" autocomplete\x3d"off"\x3e\x3cslot v-bind:fields\x3d"fields"\x3e\x3c/slot\x3e\x3c/q-form\x3e',methods:{submitForm:function(){if(this.noValidate)this.submitGo();
else{$(this.$el);var a=this;this.$refs.qForm.validate().then(function(b){b&&a.submitGo()})}},resetForm:function(){this.fields=Object.assign({},this.fieldsInitial);this.fieldsChanged={}},submitGo:function(){var a=this,b=$(this.$el),c=null,d=null,e=$(this.buttonClicked||document.activeElement);e.length&&b.has(e)&&e.is('button[type\x3d"submit"], input[type\x3d"submit"], input[type\x3d"image"]')&&(e.is("[name]")&&(c=e.attr("name"),d=e.val()),e.prop("disabled",!0),setTimeout(function(){e.prop("disabled",
!1)},3E3));var f=Object.keys(this.fields).length?new FormData:new FormData(this.$refs.qForm.$el);$.each(this.fields,function(a,b){b&&f.set(a,b)});for(b=f.entries()[Symbol.iterator]();;){var g=b.next();if(g.done)break;var m=g.value,g=m[0],m=m[1];moqui.isString(m)&&m.startsWith("__")&&f.set(g,"")}f.set("moquiSessionToken",this.$root.moquiSessionToken);c&&f.set(c,d);this.$root.loading++;var k=new XMLHttpRequest;k.open(this.method,this.$root.appRootPath+this.action,!0);k.responseType="blob";k.withCredentials=
!0;k.onload=function(){if(200===this.status){a.$root.loading--;var b=k.getResponseHeader("Content-Disposition");if(!b||-1===b.indexOf("attachment")&&-1===b.indexOf("inline")){var c=new FileReader;c.onload=function(b){b=b.target.result;try{a.handleResponse(JSON.parse(b))}catch(c){a.handleResponse(b)}};c.readAsText(this.response)}else{var d=this.response,c="";b&&-1!==b.indexOf("attachment")&&(b=/filename[^;=\n]*=((['"]).*?\2|[^;\n]*)/.exec(b),null!=b&&b[1]&&(c=b[1].replace(/['"]/g,"")));if("undefined"!==
typeof window.navigator.msSaveBlob)window.navigator.msSaveBlob(d,c);else{var e=window.URL||window.webkitURL,f=e.createObjectURL(d);c?(d=document.createElement("a"),"undefined"===typeof d.download?window.location.href=f:(d.href=f,d.download=c,document.body.appendChild(d),d.click())):window.location.href=f;setTimeout(function(){e.revokeObjectURL(f)},100)}}}else moqui.handleLoadError(this,this.statusText,"")};k.setRequestHeader("Accept","application/json");k.send(f)},handleResponse:function(a){var b=
!1;a&&moqui.isPlainObject(a)?(b=moqui.notifyMessages(a.messageInfos,a.errors),a.screenUrl&&0<a.screenUrl.length?this.$root.setUrl(a.screenUrl):a.redirectUrl&&0<a.redirectUrl.length&&(window.location.href=a.redirectUrl)):console.warn("m-form no response or non-JSON response: "+JSON.stringify(a));(a=this.submitHideId)&&0<a.length&&$("#"+a).modal("hide");(a=this.submitReloadId)&&0<a.length&&this.$root.reloadContainer(a);(a=this.submitMessage)&&a.length?(b=eval('"'+a+'"'),moqui.webrootVue.$q.notify($.extend({},
moqui.notifyOpts,{message:b})),moqui.webrootVue.addNotify(b,"success")):b||moqui.webrootVue.$q.notify($.extend({},moqui.notifyOpts,{message:"Submit successful"}))}},mounted:function(){var a=this,b=$(this.$el);this.focusField&&this.focusField.length&&b.find('[name^\x3d"'+this.focusField+'"]').addClass("default-focus").focus();b.find('button[type\x3d"submit"], input[type\x3d"submit"], input[type\x3d"image"]').on("click",function(){a.buttonClicked=this})}});
...
Vue.component("m-form-go-page",{name:"mFormGoPage",props:{idVal:{type:String,required:!0},maxIndex:Number,formList:Object},data:function(){return{pageIndex:""}},template:'\x3cq-form v-if\x3d"!formList || (formList.paginate \x26\x26 formList.paginate.pageMaxIndex \x3e 4)" @submit.prevent\x3d"goPage" :id\x3d"idVal+\'_GoPage\'"\x3e\x3cq-input dense v-model\x3d"pageIndex" type\x3d"text" size\x3d"4" name\x3d"pageIndex" :id\x3d"idVal+\'_GoPage_pageIndex\'" placeholder\x3d"Page #"   :rules\x3d"[val \x3d\x3e /^\\d*$/.test(val) || \'digits only\', val \x3d\x3e ((formList \x26\x26 +val \x3c\x3d formList.paginate.pageMaxIndex) || (maxIndex \x26\x26 +val \x3c maxIndex)) || \'higher than max\']"\x3e\x3c/q-input\x3e\x3cq-btn dense flat no-caps type\x3d"submit" label\x3d"Go"\x3e\x3c/q-btn\x3e\x3c/q-form\x3e',
...
url:this.valueUrl,data:a,dataType:"json",headers:{Accept:"application/json"},error:function(a,d,e){b.loading=!1;moqui.handleAjaxError(a,d,e)},success:function(a){b.loading=!1;var d="",e="";try{var f=JSON.parse(a);$.isArray(f)&&f.length?f=f[0]:$.isPlainObject(f)&&f.hasOwnProperty("options")&&f.options.length&&(f=f.options[0]);f.hasOwnProperty("label")&&(d=f.label);f.hasOwnProperty("value")&&(e=f.value)}catch(g){}d&&d.length||(d=a);e&&e.length||(e=a);moqui.isNumber(e)&&(e=e.toString());b.$emit("input",
e);b.fields&&b.fields.length&&b.name&&b.name.length&&(b.fields[b.name+"_display"]=d);b.curDisplay=d}})}else console.warn("In m-display tried to populateFromUrl but not hasAllParms and not dependsOptional");else console.warn("In m-display tried to populateFromUrl but no valueUrl")}},computed:{displayValue:function(){return this.curDisplay&&this.curDisplay.length?this.curDisplay:this.value}},mounted:function(){if(this.valueUrl&&this.valueUrl.length){var a=this.dependsOn,b;for(b in a)a.hasOwnProperty(b)&&
this.fields&&this.$watch("fields."+b,function(){this.populateFromUrl({term:this.value})});this.valueLoadInit&&this.populateFromUrl()}}});
Vue.component("m-drop-down",{name:"mDropDown",props:{value:[Array,String],options:{type:Array,"default":function(){return[]}},combo:Boolean,allowEmpty:Boolean,multiple:Boolean,optionsUrl:String,serverSearch:Boolean,serverDelay:{type:Number,"default":300},serverMinLength:{type:Number,"default":1},optionsParameters:Object,labelField:{type:String,"default":"label"},valueField:{type:String,"default":"value"},dependsOn:Object,dependsOptional:Boolean,optionsLoadInit:Boolean,form:String,fields:{type:Object},
...
this.value){b=!0;break}}b||(!this.allowEmpty&&!this.multiple&&a&&a.length&&a[0].value?(console.warn("checkCurrentValue setting "+this.name+" to "+a[0].value),this.$emit("input",a[0].value)):this.$emit("input",null))},optionLabel:function(a){var b=this.allOptions;if(!b||!b.length)return"";for(var c=0;c<b.length;c++){var d=b[c];if(a===d.value)return d.label}return""},removeValue:function(a){var b=this.value;if(moqui.isArray(b)){for(var c=[],d=0;d<b.length;d++){var e=b[d];e!==a&&c.push(e)}b.length!==
c.length&&this.$emit("input",c)}else console.warn("Tried to remove value from m-drop-down multiple "+this.name+" but value is not an Array")},clearAll:function(){this.$emit("input",null)}},mounted:function(){this.serverSearch&&(this.optionsUrl||console.error("m-drop-down in form "+this.form+" has no options-url but has server-search\x3dtrue"));if(this.optionsUrl&&this.optionsUrl.length){var a=this.dependsOn,b;for(b in a)if(a.hasOwnProperty(b)&&this.fields){var c=this;this.$watch("fields."+a[b],function(){c.$emit("input",
null);c.populateFromUrl({term:c.lastSearch})})}this.optionsLoadInit&&(this.serverSearch?this.value&&this.value.length&&moqui.isString(this.value)&&this.populateFromUrl({term:this.value}):this.populateFromUrl())}this.multiple||this.allowEmpty||this.value&&this.value.length||!this.options||!this.options.length||!this.dependsOn&&1!==this.options.length||this.$emit("input",this.options[0].value)}});
...
Vue.component("m-subscreens-tabs",{name:"mSubscreensTabs",data:function(){return{pathIndex:-1}},template:'\x3cdiv v-if\x3d"subscreens.length \x3e 0"\x3e\x3cq-tabs dense no-caps align\x3d"left" active-color\x3d"primary" indicator-color\x3d"primary" :value\x3d"activeTab"\x3e\x3cq-tab v-for\x3d"tab in subscreens" :key\x3d"tab.name" :name\x3d"tab.name" :label\x3d"tab.title" :disable\x3d"tab.disableLink" @click.prevent\x3d"goTo(tab.pathWithParams)"\x3e\x3c/q-tab\x3e\x3c/q-tabs\x3e\x3cq-separator class\x3d"q-mb-md"\x3e\x3c/q-separator\x3e\x3c/div\x3e',
...
computed:{navMenuItem:function(){return this.$root.navMenuList[this.menuIndex]},navMenuLength:function(){return this.$root.navMenuList.length}}});
...
b)},addNavPlugin:function(a){var b=this;moqui.loadComponent(this.appRootPath+a,function(a){b.navPlugins.push(a)})},addNavPluginsWait:function(a,b){if(a&&a.length>b){this.addNavPlugin(a[b]);var c=this;a.length>b+1&&setTimeout(function(){c.addNavPluginsWait(a,b+1)},500)}},addNotify:function(a,b){var c=this.notifyHistoryList.slice(0),d=new Date,e=d.getHours();10>e&&(e="0"+e);d=d.getMinutes();10>d&&(d="0"+d);for(c.unshift({message:a,type:b,time:e+":"+d});25<c.length;)c.pop();this.notifyHistoryList=c},
switchDarkLight:function(){this.$q.dark.toggle();$.ajax({type:"POST",url:this.appRootPath+"/apps/setPreference",error:moqui.handleAjaxError,data:{moquiSessionToken:this.moquiSessionToken,preferenceKey:"QUASAR_DARK",preferenceValue:this.$q.dark.isActive?"true":"false"}})},toggleLeftOpen:function(){this.leftOpen=!this.leftOpen;$.ajax({type:"POST",url:this.appRootPath+"/apps/setPreference",error:moqui.handleAjaxError,data:{moquiSessionToken:this.moquiSessionToken,preferenceKey:"QUASAR_LEFT_OPEN",preferenceValue:this.leftOpen?
"true":"false"}})},stopProp:function(a){a.stopPropagation()},getNavHref:function(a){a||(a=this.navMenuList.length-1);a=this.navMenuList[a];if(a.extraPathList&&a.extraPathList.length){var b=a.path+"/"+a.extraPathList.join("/"),c=a.pathWithParams.indexOf("?");0<c&&(b+=a.pathWithParams.slice(c));return b}return a.pathWithParams||a.path},getLinkPath:function(a){moqui.isPlainObject(a)&&(a=moqui.makeHref(a));this.appRootPath&&this.appRootPath.length&&0!==a.indexOf(this.appRootPath)&&(a=this.appRootPath+
a);var b=a.split("/"),c=this.appRootPath?2:1;b.length>c&&(b[c]=this.linkBasePath.slice(1),a=b.join("/"));return a},getQuasarColor:function(a){return moqui.getQuasarColor(a)}},watch:{navMenuList:function(a){if(0<a.length){var b=a[a.length-1];a=1<a.length?a[a.length-2]:null;b.extraPathList&&(this.extraPathList=b.extraPathList);var c=this.basePathSize,d=b.path.split("/").slice(c+1);console.info("nav updated fullPath "+JSON.stringify(d)+" currentPathList "+JSON.stringify(this.currentPathList)+" cur.path "+
b.path+" basePathSize "+c);this.currentPathList=d;this.reloadSubscreens();a=(a?a.title+" - ":"")+b.title;var c=b.pathWithParams,e=c.indexOf("?");if(0<e){for(var d=["pageIndex","orderBySelect","orderByField","moquiSessionToken"],f=c.substring(e+1).split("\x26"),c=c.substring(0,e),e=0,g="",m=0;m<f.length;m++){var k=f[m],c=-1===c.indexOf("?")?c+"?":c+"\x26",c=c+k;if(!(3<e)){var h=k.indexOf("\x3d");if(0<h){var l=k.substring(0,h),k=k.substring(h+1);0<l.indexOf("_op")||0<l.indexOf("_not")||0<l.indexOf("_ic")||
0<d.indexOf(l)||l===k||(0<g.length&&(g+=", "),g+=decodeURIComponent(k),e++)}}}0<g.length&&(70<g.length&&(g=g.substring(0,70)+"..."),a=a+" ("+g+")")}d=this.navHistoryList;for(f=0;f<d.length;)d[f].pathWithParams===c?d.splice(f,1):f++;for(d.unshift({title:a,pathWithParams:c,image:b.image,imageType:b.imageType});25<d.length;)d.pop();document.title=a}},currentPathList:function(a){(a=a[a.length-1])&&$(this.$el).removeClass().addClass(a)}},computed:{currentPath:{get:function(){var a=this.currentPathList,
...
moqui.localeMap[this.locale]&&(this.locale=moqui.localeMap[this.locale]);this.leftOpen="true"===$("#confLeftOpen").val();var a=$("#confDarkMode").val();this.$q.dark.set("true"===a);this.notificationClient=new moqui.NotificationClient(("https:"===location.protocol?"wss://":"ws://")+this.appHost+this.appRootPath+"/notws");var b=[];$(".confNavPluginUrl").each(function(a,d){b.push($(d).val())});this.addNavPluginsWait(b,0)},mounted:function(){$(this.$el).css("display","initial");this.setUrl(window.location.pathname+
window.location.search);this.notificationClient.registerListener("ALL");window.Notification&&"granted"!==Notification.permission&&"denied"!==Notification.permission&&Notification.requestPermission(function(a){"granted"===a?moqui.notifyMessages("Browser notifications enabled, if you don't want them use browser notification settings to block"):"denied"===a&&moqui.notifyMessages("Browser notifications disabled, if you want them use browser notification settings to allow")})}});
window.addEventListener("popstate",function(){moqui.webrootVue.setUrl(window.location.pathname+window.location.search)});
moqui.webrootRouter={resolve:function(a,b,c){a=moqui.isString(a)?moqui.parseHref(a):a;b=a.path;c=b.lastIndexOf("/");b={name:-1===c?b:b.slice(c+1),meta:{},path:b,hash:a.hash||"",query:a.query||"",params:{},fullPath:b,matched:[]};return{location:a,route:b,href:moqui.makeHref(a),normalizedTo:a,resolved:b}},replace:function(a,b,c){moqui.webrootVue.setUrl(a,null,b)},push:function(a,b,c){moqui.webrootVue.setUrl(a,null,b)}};Object.defineProperty(Vue.prototype,"$router",{get:function(){return moqui.webrootRouter}});
Object.defineProperty(Vue.prototype,"$route",{get:function(){return moqui.webrootVue.getRoute()}});