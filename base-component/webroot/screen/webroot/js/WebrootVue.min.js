...
c=null,d=$(this.buttonClicked||document.activeElement);d.length&&a.has(d)&&d.is('button[type\x3d"submit"], input[type\x3d"submit"], input[type\x3d"image"]')&&(d.is("[name]")&&(b=d.attr("name"),c=d.val()),d.prop("disabled",!0),setTimeout(function(){d.prop("disabled",!1)},3E3));var e=new FormData(this.$el);e.append("moquiSessionToken",this.$root.moquiSessionToken);$.each(this.fields,function(a,b){e.append(a,b)});b&&e.append(b,c);this.$root.loading++;$.ajax({type:this.method,url:this.$root.appRootPath+
this.action,data:e,contentType:!1,processData:!1,headers:{Accept:"application/json"},error:moqui.handleLoadError,success:this.handleResponse})}else if(!this.noValidate){var f=a.find("div.has-error input, div.has-error select, div.has-error textarea").first();f.length&&(a=f.parents("div.panel-collapse.collapse").last(),a.length?a.hasClass("in")?f.focus():(a=a.prevAll("div.panel-heading").last(),a.length?(b=a.find('a[data-toggle\x3d"collapse"]').first(),b.length?b.click():a.click(),setTimeout(function(){f.focus()},
250)):f.focus()):f.focus())}},handleResponse:function(a){this.$root.loading--;var b=!1;a&&moqui.isPlainObject(a)?(b=moqui.notifyMessages(a.messageInfos,a.errors),a.screenUrl&&0<a.screenUrl.length?this.$root.setUrl(a.screenUrl):a.redirectUrl&&0<a.redirectUrl.length&&(window.location.href=a.redirectUrl)):console.warn("m-form no response or non-JSON response: "+JSON.stringify(a));(a=this.submitHideId)&&0<a.length&&$("#"+a).modal("hide");(a=this.submitReloadId)&&0<a.length&&this.$root.reloadContainer(a);
(a=this.submitMessage)&&a.length?(b=eval('"'+a+'"'),$.notify(new moqui.NotifyOptions(b,null,"success",null),moqui.notifyOpts),moqui.webrootVue.addNotify(b,"success")):b||$.notify(new moqui.NotifyOptions("Submit successful",null,"success",null),moqui.notifyOpts)},fieldChange:function(a){a=a.delegateTarget;var b=$(a);b.hasClass("input-group")&&b.children("input").length&&(b=b.children("input").first(),a=b.get(0));var c=!1;if("INPUT"===a.nodeName||"TEXTAREA"===a.nodeName)c="radio"===b.attr("type")||
"checkbox"===b.attr("type")?a.checked!==a.defaultChecked:a.value!==a.defaultValue;else if("SELECT"===a.nodeName)if(a.multiple)for(var d=a.options.length,e=0;e<d;e++){var f=a.options[e];if(f.selected!==f.defaultSelected){c=!0;break}}else c=!a.options[a.selectedIndex].defaultSelected;c?(this.fieldsChanged[b.attr("name")]=!0,b.parents(".form-group").children("label").addClass("is-changed"),b.parents(".form-group").find(".select2-selection").addClass("is-changed"),b.addClass("is-changed")):(this.fieldsChanged[b.attr("name")]=
!1,b.parents(".form-group").children("label").removeClass("is-changed"),b.parents(".form-group").find(".select2-selection").removeClass("is-changed"),b.removeClass("is-changed"))}},mounted:function(){var a=this,b=$(this.$el);this.noValidate||b.validate({errorClass:"help-block",errorElement:"span",highlight:function(a,b,e){$(a).parents(".form-group").removeClass("has-success").addClass("has-error")},unhighlight:function(a,b,e){$(a).parents(".form-group").removeClass("has-error").addClass("has-success")}});
b.find('[data-toggle\x3d"tooltip"]').tooltip({placement:"auto top"});this.focusField&&0<this.focusField.length&&b.find('[name^\x3d"'+this.focusField+'"]').addClass("default-focus").focus();b.find(":input").on("change",this.fieldChange);b.find("div.input-group.date").on("change",this.fieldChange);b.find('button[type\x3d"submit"], input[type\x3d"submit"], input[type\x3d"image"]').on("click",function(){a.buttonClicked=this})}});
...
Vue.component("subscreens-tabs",{data:function(){return{pathIndex:-1}},template:'\x3cul v-if\x3d"subscreens.length \x3e 0" class\x3d"nav nav-tabs" role\x3d"tablist"\x3e\x3cli v-for\x3d"tab in subscreens" :class\x3d"{active:tab.active,disabled:tab.disableLink}"\x3e\x3cspan v-if\x3d"tab.disabled"\x3e{{tab.title}}\x3c/span\x3e\x3cm-link v-else :href\x3d"tab.pathWithParams"\x3e{{tab.title}}\x3c/m-link\x3e\x3c/li\x3e\x3c/ul\x3e',computed:{subscreens:function(){if(!this.pathIndex||0>this.pathIndex)return[];
